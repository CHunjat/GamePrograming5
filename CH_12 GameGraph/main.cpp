...
class Spire // spire 만들고 싶다
{
	...
	// 가변 배열(vector) 노드를 저장하는 컨테이너 선언
 private:
		vector<Node> nodes;
		vector<vector<int>> adj;  //인접행렬 (각 점이 어디에 연결되어 있는지 저장하는 컨테이너)
		vector<bool> visited;
		int nodeCount;
...
public:
	Spire() :nodeCount(0) {}
	void addNode(string _name, int x = 0, int y = 0) // id와 nodecount 일치하게 만듦
	{
		nodes.emplace_back(_name, nodeCount, x, y);   //nodes 컨테이너 데이터를 추가합니다.
		adj.push_back(vector<int>());				  //노드를 추가할 때 인접행렬의 공간을 생정
		visited.push_back(false);                    //방문체크공간을 생성
		nodeCount++;
	}
	...
	private:
		void calculateNodePos() //추가해준 노드들을 레벨별로 좌표를 설정하는 코드
		{
			...
			processed[0] = true; //시작지점을 방문했습니다.

			int currentLevel = 0;
			while (currentLevel < levels.size()) //각 층의 방의 갯수를 측정하는 코드
			{
				vector<int> nextLevel; //공간만들기

				for (int node : levels[currentLevel]) //0번노드에서 시작을 하라 코드
				{
				...
				currentLevel++; //다음층을 계산하세요
			}

			//모든 층의 레벨을 계산을 했으면, 각 노드가 몇 층에 있고, 면 번째에 있는지 기록이 됩니다.

			for (int level = 0; level < levels.size(); level++)
			{
				int nodeLevel = levels[level].size(); // 각 층의 방의 갯수
				int spacing = CONSOLEWIDTH / (nodeLevel + 1);   // couns consolewidth = 80

				for (int i = 0; i < nodeLevel; i++)
				{
					int nodeId = levels[level][i];			//n층 n번째 노드 = id
					nodes[nodeId].x = spacing * (i + 1);    //i가 0부터 시작되기 때문에 1을 더한다.
...
int main()
{
	...
	spire.addNode("2층 엘리트방");
	spire.addNode("2층 전투방");
	spire.addNode("2층 전투방");
	spire.addNode("2층 이벤트");
	spire.addNode("2층 휴식");
	...
	vector<int> temp = spire.findShortestPath(0, 12);
